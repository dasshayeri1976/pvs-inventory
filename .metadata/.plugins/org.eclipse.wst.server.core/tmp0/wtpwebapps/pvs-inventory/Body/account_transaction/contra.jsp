...
function show()
{
	...
	//s1=s1.replace('&','*');
	var url = "./Body/account_transaction/getsrno.jsp";
...
function codeAdd04() 
{
	...
	{			
		...
		//document.getElementsByName("partcrbal")[i].value="bal: "+x2[i];		
	}
}
</script>
<script>
...
function ccode1()//geting current bal of on a/c
{
	...
	/*var s2=document.getElementById("part1").value;//get on a/c for particular list
	var url1 = "./Body/master/getcrbal2.jsp";
	url1 += "?count=" + s2;*/
	
	xmlHttp001.onreadystatechange = codeAdd05;
	xmlHttp001.open("GET", url, true);
	//xmlHttp001.open("GET", url1, true);
	xmlHttp001.send(null);
	
	/*xmlHttp002.onreadystatechange = codeAdd06;
	xmlHttp002.open("GET", url1, true);
...
<body onload="show();partlist();getnarration();ccode1();ccode();showdraft()"><center>
</s:if>
<s:else>
<body onload="ccode();ccode1();showdraft()"><center>
</s:else>
<div>
<ul id="breadcrumbs">
...
<div style="position:fixed;top:160px;bottom: 45px;overflow-y:auto;width:100%" align="center">    
  ...
  <legend style="color:red;"><h3>Account Contra</h3></legend>
  <div align="left">
  <s:label>Serial No:</s:label>&nbsp;<s:textfield name="formbean.srno" id="srno" style="border:0px" readonly="true"/>
  </div>
  
  
  <s:form action="contracreation" namespace="/" method="post" autocomplete="off">
 ...
 <br>
  ...
  <table width="100%" border="0" id="table">
  	   ...
  	   <tr>
  	   		...
  	   		<td width="12.5%" style="padding-right:5px"><s:select name="formbean.partforcontra" id="partforcontra" list="usList4" listKey="part1" value="formbean.part1" listValue="part1" cssStyle=" border:1px solid #ccc;height:20px;background:#FFFF99;width:150px;" onchange="ccode();"></s:select></td>
  	  		...
  	  		<td width="12.5%" style="padding-right:5px"><s:textfield name="formbean.narration" id="narration" size="25" cssStyle="height:25px;border:1px solid #ccc;background:#FFFF99"></s:textfield></td>
  	   </tr>
  	   </s:if>
  	   <s:else>
  	   <%int i=0; %>
  	   ...
  	   <tr>
  	   		<td width="12.5%" style="padding-right:5px"><input type="checkbox" name="chk"></input></td>
  	   		<td width="12.5%" style="padding-right:5px"><s:select name="formbean.partforcontra" id="partforcontra" list="usList4" listKey="part1" value="formbean.quantity[#status.index]" listValue="part1" cssClass="part" cssStyle=" border:1px solid #ccc;height:20px;background:#FFFF99;width:150px;" onchange="ccode();"></s:select> <input type="hidden" name="formbean.contra1" class="contra1" id="contra1"></input></td>
  	   		<td style="padding-right:5px"><div style="width:110px"><s:textfield name="partcrbal" id="partcrbal" style="color:red;background:transparant;border:0;" readonly="true"></s:textfield></div></td>
  	   			   
  	   		<td width="12.5%" style=""><s:textfield name="formbean.credit[%{#status.count-1}]" id="credit" size="25" cssClass="sourav" cssStyle="height:25px;border:1px solid #ccc;background:#FFFF99" onkeyup="test12a();"></s:textfield> <input type="hidden" name="formbean.credithidden" class="credithidden" id="credithidden"></input></td>	   	 	
  	  		<td width="12.5%" style="padding-right:5px"><s:textfield name="formbean.narration[%{#status.count-1}]" id="narration" size="25" cssStyle="height:25px;border:1px solid #ccc;background:#FFFF99" cssClass="nar"></s:textfield> <input type="hidden" name="formbean.narration1" class="narration1"/></td>
  	   </tr>
  	   <%i++; %>
  	   </s:iterator>
  	   </s:else>
  ...
  <div align="right">
  <label>Total Amount:- </label><s:textfield id="totalam" name="formbean.total" readonly="true" style="border:none;color:red"></s:textfield>
  </div>
 ...
 <a href="ledgerlist" accesskey="c"></a><!-- create ledger hot key-->
    ...
    <a href="<s:url action="contraupdate" namespace="/" ><s:param  name="id" value="%{#attr.table12.id}"></s:param></s:url>" style="color:  #fff;text-decoration:none;font-size:12px;" class="classname" >Update</a>
    </display:column>
    
    <display:column title="Delete" media="html" style="text-align: center;width:8%;background-color:#ecf2f6;" >
    <a href="<s:url action="contradelete" namespace="/" ><s:param  name="id" value="%{#attr.table12.id}"></s:param></s:url>" style="color:  #fff;text-decoration:none;font-size:12px;" class="print" onclick="return confirmation();">Delete</a>
    </display:column>
    
    <display:column title="Print" media="html" style="text-align: center;width:8%;background-color:#ecf2f6;" >
    <a href="./Body/transaction_print/contraprint.jsp?id=<s:property value="%{#attr.table12.id}" />" style="color:  #fff;text-decoration:none;font-size:12px;" class="classname" onclick="">Print</a>
    </display:column>
	</s:if>
	<s:else>
