...
function show()
{
	...
	//s1=s1.replace('&','*');
	var url = "./Body/transaction/getsrnoforjournal.jsp";
...
function codeAdd044() 
{
	...
	{			
		...
		document.getElementById("srno").value=x1;		
	}
}
...
function ccode()//geting current bal 
{
	...
	val.join(',')
	//s1=s1.replace('&','*');
	var url = "./Body/transaction/getcrbal.jsp";
...
function showdrawn()
{
	...
	//s1=s1.replace('&','*');
	var url = "./Body/transaction/showdrawn.jsp";
	url += "?count=" + s1; // + "&sec=" + s;
	//url +="?count=" +str+"&date1="+s;
...
function todummy()
{
	...
	//alert(ar);
	ar.join(",");
	var url = "./Body/transaction/inserttodummy.jsp";
	url += "?count=" + ar.join(",");
...
function showpopup()
{
	...
	var url = "./Body/transaction/checkforsu.jsp";
...
function loadnew()
{
	window.location="journalregisterlistdetails1";
}
</script>
<style>
#popup
...
<s:actionmessage/>
  ...
  <table width="100%" border="" id="table">
  	   ...
  	   <tr>
  	   		<td style=""><input type="checkbox" name="chk"></input></td>
  	   		<td width="12.5%" style=""><s:select name="formbean.dorc" id="dorc" list="#{'':'', 'dr':'Dr', 'cr':'Cr'}" value="formbean.dorc[#status.index]" cssClass="dorc" cssStyle="border:1px solid #ccc;background:#FFFF99;width:50px;" onchange="fordrcr();"></s:select> <input type="hidden" name="formbean.dorchidden" class="dorchidden" id="dorchidden"/></td>
 ...
 <s:textarea name="formbean.desc1" id="desc1" cols="53" rows="4" maxlength="275" cssStyle="border:1px solid #ccc;background:#FFFF99;resize:none;border:0px solid"></s:textarea>
  <a href="ledgerlist" accesskey="c"></a><!-- create ledger hot key-->
       ...
       <div align="right" style="width:99%;position:relative;  padding:7px;">
      	...
      	<input type="button" name="formbean.back" value="Back" Class="butStnd" onclick="loadnew();" ></input>
    ...
    <a href="<s:url action="journalupdate" namespace="/" ><s:param  name="id" value="%{#attr.table.id}"></s:param></s:url>" style="color:  #fff;text-decoration:none;font-size:12px;" class="classname">Update</a>
    </display:column>
    
    <display:column title="Delete" media="html" style="text-align: center;width:8%;background-color:#ecf2f6;" >
    <a href="<s:url action="journaldelete" namespace="/" ><s:param  name="id" value="%{#attr.table.id}"></s:param></s:url>" style="color:  #fff;text-decoration:none;font-size:12px;" class="print" onclick="return confirmation();">Delete</a>
    </display:column>
    
     <display:column title="Print" media="html" style="text-align: center;width:8%;background-color:#ecf2f6;" >
    <a href="./Body/transaction_print/journalprint.jsp?id=<s:property value="%{#attr.table.id}" />" style="color:  #fff;text-decoration:none;font-size:12px;" class="classname" onclick="">Print</a>
    </display:column>
	</s:if>
	<s:else>
